...
import com.im.sell.converter.OrderMaster2OrderDTOConverter;
...
//        OrderMaster orderMaster = orderMasterRepository.getOne(orderId);
        Optional<OrderMaster> one = orderMasterRepository.findById(orderId);
        System.out.println(one.isPresent());
//        if (orderMaster == null){
        if (!one.isPresent()){
            throw new SellException(ResultEnum.ORDER_NOT_EXIST);
        }
        OrderMaster orderMaster = one.get();
